...
fn parsearg_op(opts: getopts::Matches,  config:&mut config_t) -> i64
{
		use self::operations::*;
		/* operations */
		if opts.opt_present("D"){
			//if(dryrun) break;
			config.op = match config.op {
...
//  */
fn parseargs(argc: usize, argv: Vec<String>, config: &mut config_t) -> u8
{
	let opt; //int opt;
	let option_index = 0;// int option_index = 0;
	let result;// int result;
	let optstring = "DFQRSTUVb:cdefghiklmnopqr:stuvwxy";// const char *optstring = "DFQRSTUVb:cdefghiklmnopqr:stuvwxy";
	let mut opts = getopts::Options::new();
	{
		...
		opts.optopt("", "--dbpath", "", "");
		opts.optflag("", "--cascade", "");
		opts.optflag("", "--changelog", "");
		opts.optflag("", "--clean", "");
		opts.optflag("", "--nodeps", "");
		opts.optflag("", "--deps", "");
		opts.optflag("", "--explicit", "");
		opts.optflag("", "--groups", "");
		opts.optflag("", "--info", "");
		opts.optflag("", "--check", "");
		opts.optflag("", "--list", "");
		opts.optflag("", "--foreign", "");
		opts.optflag("", "--native", "");
		opts.optflag("", "--nosave", "");
		opts.optflag("", "--owns", "");
		opts.optflag("", "--file", "");
		...
		opts.optopt("", "--root", "", "");
		opts.optopt("", "--sysroot", "", "");
		opts.optflag("", "--recursive", "");
		opts.optflag("", "--search", "");
		opts.optflag("", "--regex", "");
		opts.optflag("", "--machinereadable", "");
		opts.optflag("", "--unrequired","");
		opts.optflag("", "--upgrades", "");
		opts.optflag("", "--sysupgrade","");
		opts.optflag("", "--unneeded", "");
		opts.optflag("", "--verbose", "");
		opts.optflag("", "--downloadonly", "");
		opts.optflag("", "--refresh", "");
	...
	let matches = match opts.parse(&argv[1..]) {
        Ok(m) => m,
        Err(f) => {println!("Hee");return 1;}
    };
	parsearg_op(matches, config);



	if config.op.is_none() {
		//TODO: implement this
...
//
// 	/* parse all othe	while()(opt = getopt_long(argc, argv, optstring, opts, &option_index)) != -1) {
// 		if(opt == 0) {
// 			continue;
// 		} else if(opt == '?') {
...
pub fn main()
{
	...
	let mut ret = 0;
	let mut config;
	// let myuid = getuid();//uid_t myuid = getuid();
...
// #endif
	...
	ret = parseargs(argc, argv, &mut config);
	// if(ret != 0) {
	// 	cleanup(ret);
	// }
    //
